...
public class Day27LectureApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		// Person updatePerson = new Person("65a89fee6c4c6e0a3a398158", "Nicole Kidman", 45, "F", Arrays.asList("RC", "Swimmming", "Cycling"));
		...
		// day 27 - slide 19
		// TextCriteria textCriteria = TextCriteria.forDefaultLanguage().matchingPhrase("jaMEs");

		// TextQuery textQuery = TextQuery.queryText(textCriteria);

		// List<Person> resultTextQuery = mt.find(textQuery, Person.class, "persons");

		// System.out.println("Slide 19 (non case-sensitive): " + resultTextQuery.toString());

		// textCriteria = TextCriteria.forDefaultLanguage().matchingPhrase("jaMEs").caseSensitive(true);

		// textQuery = TextQuery.queryText(textCriteria);

		// List<Person> resultTextQuery2 = mt.find(textQuery, Person.class, "persons");

		// System.out.println("Slide 19 (case-sensitive): " + resultTextQuery2.toString());

		// // day 27 - slide 22
		// textCriteria = TextCriteria.forDefaultLanguage().matchingPhrase("swimmming");

		// textQuery = TextQuery.queryText(textCriteria).includeScore("score").sortByScore();

		// List<Document> resultTextQuery3 = mt.find(textQuery, Document.class, "persons");

		// // doesn't affect much due low number of records
		// System.out.println("Slide 22 (non case-sensitive): " + resultTextQuery3.toString());
